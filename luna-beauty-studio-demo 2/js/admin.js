...
/* Safe state load — never let a corrupt payload kill the admin */
function safeState() {
  try { return LunaData.loadState(); }
  catch (e) {
    try { return LunaData.resetState(true); }
    catch (e2) { return LunaData.resetState(false); }
  }
}
let state = safeState();
...
/* ---------- Boot ---------- */
function boot() {
  if (document.body.dataset.adminBooted === '1') return;
  document.body.dataset.adminBooted = '1';

  let authed = false;
  try { authed = sessionStorage.getItem('luna_demo_admin') === '1'; } catch (e) { authed = false; }
...
/* ---------- Start ---------- */
/* Run now if the DOM is already parsed (script loaded late or cached),
   otherwise wait for it. Prevents the "blank admin" failure mode. */
